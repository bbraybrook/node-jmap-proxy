          ...
          selectPromise.then(function(box){
            ...
            var configPromise = new Promise(function(yay2,nay2){
              if (data.update[id].role || data.update[id].sortOrder) {
                ...
                if (!userConfig.folders) userConfig.folders = {};
                if (!userConfig.folders[id]) userConfig.folders[id] = {};
                if (data.update[id].role) userConfig.folders[id].role = data.update[id].role;
                if (data.update[id].sortOrder) userConfig.folders[id].sortOrder = data.update[id].sortOrder;
                state.active[token].config = userConfig;
                myConfig.set(user,userConfig).then(function(confRes){
                  if (confRes) {
