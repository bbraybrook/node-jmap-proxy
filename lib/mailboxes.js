...
exports.getMailboxes = function(token,data,seq,res) {
  ...
  imap.getBoxes(function(err,boxes){
    ...
    iterate_getMailboxes(response,boxes,null,token); // sync
      ...
      if (response.list[i].mayReadItems == true) {
        ...
        var promise = new Promise(function(resolve, reject){
          ...
          imap.openBox(pid,function(err,box){
            if (err) {
              resolve({'id':pid,'i':pi,'err':err});
            } else if (box.messages) {
        ...
        promise.then(function(obj){
          if (obj.err) {
            console.log('err selecting '+obj.id);
          } else {
...
iterate_getMailboxes = function(response,boxes,parentmb,token) {
  var conf = state.active[token].config;
  for (var mailbox in boxes) {
    ...
    if (mailbox == 'INBOX' && !parentmb) {
      if (conf.mailboxes && conf.mailboxes[entry.id] && conf.mailboxes[entry.id].sortOrder) {
    ...
    if (obj.children) {
      iterate_getMailboxes(response,obj.children,entry.id,token)
    }
  }
};
