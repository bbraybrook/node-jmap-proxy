  ...
  return new Promise(function(yay, nay) {
    ...
    idlist.forEach(function(id){
      ...
      promise = promise.then(function() {
        var myuid = uid;
        return exports.getMessage(token,folder,uid);
      }).then(function(result){
        messages.push(result);
      }).catch(function(err){
        console.log('err fetching message: '+err);
      });
    });
...
exports.copyMessages = function(token,data,seq,res) {
  var imap = state.active[token].imap;
  var user = data.fromAccountId || state.active[token].username;
  var response = {'created':{}, 'notCreated':{}, 'fromAccountId':user};
  // not implemented yet - need multi account support
  res.status('201').send(JSON.stringify([['copiedMessages',response,seq]]));
}

exports.reportMessages = function(token,data,seq,res) {
  var imap = state.active[token].imap;
  var user = data.accountId || state.active[token].username;
  var response = {'reported':[], 'notFound':[], 'asSpam':data.asSpam, 'accountId':user};
  // must be sequential, as messages could be from different mailboxes
  var promise = Promise.resolve(null);
  data.messageIds.forEach(function(messageId) {
    var a = util.decodeId(messageId);
    var mailbox = a[0];
    var uid = a[1];
    exports.getMessage(token,mailbox,uid).then(function(msg){
      // TODO: send the message to another service
      response.reported.push(messageId);
    });
  });
  // not implemented yet - need multi account support
  res.status('201').send(JSON.stringify([['messagesReported',response,seq]]));
}
